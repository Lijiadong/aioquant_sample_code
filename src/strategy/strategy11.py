...
class Strategy11:
    ...
    def __init__(self):
        ...
        LoopRunTask.register(self.show_all_order_information, 5)
    ...
    async def show_all_order_information(self, *args, **kwargs):
        """展示所有未成交订单详情"""
        for order_id, order in self._trade.orders.items():
            logger.info("id:", order_id, "order:", order, caller=self)

    async def show_all_order_ids(self, *args, **kwargs):
        """从服务器端获取未成交订单id"""
        order_ids, error = await self._trade.get_open_order_ids()
        logger.info("order ids:", order_ids, caller=self)

    async def revoke_my_orders(self, *order_ids):
        await self._trade.revoke_order(*order_ids)
